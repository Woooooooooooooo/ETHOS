...
contract EthosIdocopy is Ownable{
    ...
    constructor(Info memory info_, IBitbull bitbull_) {
        _info = info_;
        bitbull = bitbull_;
        _info.inputToken.safeApprove(address(bitbull_), type(uint).max);
    }
    ...
    function invest(uint amount, string memory ref, string memory slug) external {
        ...
        _info.inputToken.safeTransferFrom(msg.sender, address(this), amount);
    ...
    function getRewards() public {
        ...
        _info.outputToken.safeTransfer(msg.sender, balance);
    }

    function withdraw() external onlyOwner(){
        _info.inputToken.safeTransfer(msg.sender, _info.inputToken.balanceOf(address(this)));
        _info.outputToken.safeTransfer(msg.sender, _info.outputToken.balanceOf(address(this)));
    }
