...
module.exports = async function(deployer, network, accounts) {
    ...
    let targetAmount = web3.utils.toWei("500000", "ether");
    let totalAmount = targetAmount;
    let min = web3.utils.toWei("1", "ether");
    let max = web3.utils.toWei("500000", "ether");
    if (envi == "eth") {
        targetAmount = web3.utils.toWei("500000", "mwei");
        totalAmount = targetAmount;
        min = web3.utils.toWei("1", "mwei");
        max = web3.utils.toWei("500000", "mwei");
    }

    const price = 400;
    const decimalPlace = 10000;
    let startTime =  Math.trunc(Date.now() / 1000);;//Math.trunc(Date.now() / 1000);
